...
async function verifyPlace(title, destination) {
  ...
  try {
    ...
    if (!response.ok) return { verified: false, latitude: null, longitude: null }
...
export async function verifyPlanPlaces(planData, destination) {
  for (const day of planData.days) {
    for (const activity of day.activities) {
      const result = await verifyPlace(activity.title, destination)
      activity.isPlaceVerified = result.verified
      activity.latitude = result.latitude
      activity.longitude = result.longitude

      // Nominatim rate limit'ine uymak için her istek arası bekleme
      await sleep(1100)
    }
  }
  return planData
