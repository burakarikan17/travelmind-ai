...
import { buildCacheKey, getCachedPlan, setCachedPlan } from '../services/cacheService.js'
...
router.post('/generate-plan', requireAuth, generatePlanLimiter, async (req, res) => {
  ...
  try {
    ...
    const verifiedPlan = await verifyPlanPlaces(planData, formData.destination)

    // 4. Supabase'e kaydet (trips + trip_days + trip_activities)
    ...
    // 6. Sonucu döndür
    return res.json({ tripId: trip.id, cached: false, ...verifiedPlan })
  } catch (err) {
